...
class DataGeneration():
    def __init__(self, z, hazard, model_type = 'Cox'):
        self.z = z
        self.hazard = hazard
        self.N = z.shape[0]
    ...
    def failureTime(self):
        ...
        if self.model_type == 'AH':            
            failureTime = np.random.exponential(size=self.N, scale=1 / self.hazard)
        if self.model_type == 'TM':            
            failureTime = np.exp(logistic.rvs(size = self.N) + self.z[:,0] - 0.1 * np.exp(self.z[:,1]) + 0.2 * self.z[:,0] * self.z[:,1])
        return failureTime

    # censoring time
    def censoringTime(self, mu):
        temp = np.random.exponential(size=self.N, scale=1 / mu)
        result = np.clip(temp, None, 6)  # set the length of study is 6
        return result
    ...
    def timeAndControls(self, failureTime, censoringTime):
        ...
        for i in range(n):
            if failureTime[i] >= censoringTime[i]:
                ...
                indicator.append(1)
            result.append(temp)
        censoringPercent = (n - count) / n
        return np.array(result), np.array(indicator), censoringPercent

    def getData(self, censoringRate):
        if self.model_type == 'Cox':
...
class Simulation(multiprocessing.Process):
    ...
    def run(self):     
        for seed in self.seed_list:   
            ...
            print(self.model_type + ' :' + str(seed))
            saveFilename = self.saveFolder + 'result_seed_'+'_'+str(seed)+'.pickle'
                        
            with open(saveFilename, 'wb') as file:            
                # Data generation
                if self.model_type == 'Cox':                    
                    z_true =  multivariate_normal.rvs(mean = np.zeros(2), cov = np.array([[1,0.5],[0.5,1]]), size = self.N)
                    z = np.random.randn(self.N, 2)
                    hazard = np.exp(z_true.dot(np.array([1, -1])))  # lambda_0=1, T follows exponential                                 
                if self.model_type == 'AH':
                    z_true =  multivariate_normal.rvs(mean = np.zeros(2), cov = np.array([[1,0.5],[0.5,1]]), size = self.N)
                    z = np.random.randn(self.N, 2)                    
                    hazard = 2 + np.clip(z_true, -3, 3).dot(np.array([1, -1])) * 0.3 # lambda_0=1, T follows exponential
                if self.model_type == 'TM':
                    z_true =  multivariate_normal.rvs(mean = np.zeros(2), cov = np.array([[1,0.5],[0.5,1]]), size = self.N)
                    z = np.random.randn(self.N, 5)
                    hazard = None
                fullData = DataGeneration(z=z_true, hazard=hazard, model_type=self.model_type).getData(self.censoringRate)
                z = pd.DataFrame(z)
...
if __name__ == '__main__':
    ...
    subsamplingRate = 0.1
    numPilotSample = 100
    for model_type in ['Cox', 'AH', 'TM']: 
        processlist = []
            
        saveFolder = './Optimal_sampling_misspecified/'+ model_type + '_'  + str(numPilotSample) + '/'        
        if(not os.path.exists(saveFolder)):
            os.makedirs(saveFolder)
            
        for i in range(1):
            seed_list = [int(k) for k in (np.arange(1) + i * 10)]
            processlist.append(Simulation(N = N, 
                                        seed_list = seed_list,
